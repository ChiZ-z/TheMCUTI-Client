...
import { History } from 'src/app/core/models/history.model';
import { Constants, UtilService, EnumHelper, TokenService, Timings } from 'src/app/core';
...
@Component({
  selector: 'tmc-project-history-card',
  templateUrl: './project-history-card.component.html',
  styleUrls: ['./project-history-card.component.css'],
  animations: [
    trigger('slideIn', [
...
export class ProjectHistoryCardComponent implements OnInit {
  ...
  goToTerm() {
    this.router.navigateByUrl('/projects/' + this.history.project.id + '/terms?page=1&search=' + this.history.term.termValue);
  }
  ...
  showImportedValues() {
    ...
    this.historyService.getChildHistoryEvents(this.projectId, this.history.id).subscribe((data: History[]) => {
